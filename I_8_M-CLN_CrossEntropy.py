...
class IQ_ECA_CLDNN(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        DNN_input = lstm_layer_2_output[:, -1, :]  #(128, 50)
...
def train(AP_model,IQ_model, Assistant_DNN,Multitask_classifier,train_loader, val_loader,
    ...
    for epoch in range(epochs):
        ...
        for X_IQ, targets,labels_smooth,yi_s in train_loader:
            ...
            IQ_loss = F.cross_entropy(IQ_outputs, targets)
            ...
            AP_loss = F.cross_entropy(AP_outputs, targets)
...
#               print(f'Multitask_classifier_output:{Multitask_classifier_output.shape}')
            Multitask_classifier_loss = F.cross_entropy(Multitask_classifier_output, targets)
            # 先不考虑中间辅助层梯度的影响。
...
#                print(f'input_noisy_signal_in_val:{input_noisy_signal_in_val.shape},IQ_out_val:{IQ_out_val.shape},IQ_outputs_val:{IQ_outputs_val.shape}')
                IQ_loss = F.cross_entropy(IQ_outputs_val, targets)
...
#                print(f'X_AP_val:{X_AP_val.shape},AP_out_val:{AP_out_val.shape},outputs:{outputs.shape}')
                AP_loss = F.cross_entropy(outputs, targets)
...
#                print(f'val_Multitask_classifier_output:{val_Multitask_classifier_output.shape}')
            # print(f'IQ_outputs:{IQ_outputs.shape},AP_outputs:{AP_outputs.shape},\nassistant_DNN_output:{assistant_DNN_output.shape}'
                ...
                val_Multitask_classifier_loss = F.cross_entropy(val_Multitask_classifier_output,targets)
        ...
        print(f"M-LN_CrossEntropy,Epoch {epoch + 1}/{epochs}, Train Loss: {epoch_train_loss:.4f}, "
    ...
    torch.save(loss_data, 'D:/sunfengkui/M-CLN_CrossEntropy/ME-CLN_CrossEntropy_loss_data.pth')
...
def test(AP_model,IQ_model,Assistant_DNN,Multitask_classifier,test_loader):
    ...
    with torch.no_grad():
        ...
        for X_IQ, targets,labels_smooth,yi_s,snr_labels in test_loader:
            ...
            Multitask_classifier_loss = F.cross_entropy(Multitask_classifier_out,targets)
        ...
        for snr, accuracy in accuracy_per_snr.items():
            (f"SNR: {snr}dB, Accuracy: {accuracy:.4f}")
            ...
            torch.save(features, f"D:/sunfengkui/M-CLN_CrossEntropy/TSNE/all_features of {snr}dB,lr={lr},epoch={epoch}128_5.pt")
            torch.save(all_targets, f"D:/sunfengkui/TSNE/all_targets of {snr}dB,lr={lr},epoch={epoch}128_5.pt")
            tsne = TSNE(n_components=2, perplexity=30.0, n_iter=1000)
        torch.save(snr_targets, f"D:/sunfengkui/M-CLN_CrossEntropy/confusion_matrix/confusion_matrix_snr_targets.pt")
        torch.save(snr_predictions, f"D:/sunfengkui/M-CLN_CrossEntropy/confusion_matrix/confusion_matrix_snr_predictions.pt")

    torch.save(AP_model.state_dict(),
               f"D:/sunfengkui/M-CLN_CrossEntropy/model_state_dict/AP_model.pth")
    torch.save(IQ_model.state_dict(),
               f"D:/sunfengkui/M-CLN_CrossEntropy/model_state_dict/IQ_model.pth")
    torch.save(Assistant_DNN.state_dict(),
               f"D:/sunfengkui/M-CLN_CrossEntropy/model_state_dict/Assistant_DNN_model.pth")
    torch.save(Multitask_classifier.state_dict(),
               f"D:D:/sunfengkui/M-CLN_CrossEntropy/model_state_dict/Multitask_classifier.pth")
